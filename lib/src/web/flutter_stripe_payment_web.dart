...
import 'js/stripe-js/elements/base.dart';
...
class StripePaymentPlugin {
  Stripe _stripe;
  StripeElements _elements;
  StripeElementBase _card;
  PaymentRequestTokenEvent _tokenEvent;
  html.Element _stripeElement;
  static get platformVersion => "0.1.3";

  static void registerWith(Registrar registrar) {
    final MethodChannel channel = MethodChannel(
        'stripe_payment', const StandardMethodCodec(), registrar.messenger);
  ...
  Future<dynamic> handleMethodCall(MethodCall call) async {
    ...
    switch (call.method) {
      case "cardForm":
        try {
          html.Element platformView, stripeElement;

          if (_elements == null) {
            _elements = _stripe.elements(call.arguments['options']);
          }
          if (_card == null) {
            _card = _elements.create('card', call.arguments["card"]);
          }

          platformView = html.document.querySelector("flt-platform-view");
          while (platformView == null) {
            print('Retry platform view query...');
            await Future.delayed(Duration(milliseconds: 100));
            platformView = html.document.querySelector("flt-platform-view");
          }

          stripeElement = platformView.shadowRoot
              .querySelector("#${call.arguments["selector"]}");
          while (stripeElement == null) {
            print('Retry stripeElement query...');
            await Future.delayed(Duration(milliseconds: 100));
            stripeElement = platformView.shadowRoot
                .querySelector("#${call.arguments["selector"]}");
          }

          _card.mount(stripeElement);
          _stripeElement = stripeElement;
          return true;
        } catch (e) {
          print(e);
          return false;
        ...
        break;
      case "tokenFromCardForm":
        Token token = await _stripe.createToken(_card);
        return token;
        break;
      ...
      case "canMakeNativePayPayments":
        final pr = _stripe.paymentRequest(PaymentRequestOptions(
            ...
            total: PaymentRequestItem(label: 'Total', amount: 1),
      ...
      case "paymentRequestWithNativePay":
        ...
        final pr = _stripe.paymentRequest(PaymentRequestOptions(
          country: (call.arguments['country_code'] as String).toUpperCase(),
          currency: (call.arguments['currency_code'] as String).toLowerCase(),
          total: PaymentRequestItem(label: 'Total', amount: total),
...
//            requestPayerPhone: true
        ...
        throw PlatformException(
            code: 'unavailable',
            message: 'Native pay is not configured or available');
      ...
      case "confirmPaymentIntent":
        final result = await _stripe.confirmCardPayment(
            ...
            ConfirmCardPaymentData(
                payment_method: call.arguments['paymentMethodId'] ??
                    call.arguments['paymentMethod']),
            ConfirmCardPaymentOptions(handleActions: true));
        return PaymentIntentResult(paymentIntentId: result.paymentIntent?.id)
            .toJson();
  ...
  plugin_token.Token _tokenFromEvent(PaymentRequestTokenEvent event) =>
      plugin_token.Token(
          tokenId: event.token.id,
          bankAccount: event.token.bank_account == null
              ? null
              : plugin_token.BankAccount(
                  accountHolderName:
                      event.token.bank_account.account_holder_name,
                  accountHolderType:
                      event.token.bank_account.account_holder_type,
          ...
          card: event.token.card == null
              ? null
              : plugin_token.CreditCard(
